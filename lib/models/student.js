...
var StudentSchema = new Schema({
	...
	dorm: {
		type: String,
		trim: true,
		match: [/^\d{1,2}[абвгдАБВГД]{0,1}$/, 'Полето "Блок" трябва да е в правилен формат.'],
		required: [true, 'Полето "Фамилия" е задължително.']
	},
	room: {
		type: String,
		trim: true,
		match: [/^\d{1,4}[абвгдАБВГД]{0,1}$/, 'Полето "Стая" трябва да е в правилен формат.'],
		required: [true, 'Полето "Стая" е задължително.']
	},
	social: {
		type: String,
		required: [true, 'Полето "Контакти" е задължително.']
	},
	creationDate: {
		type: Date
	}
